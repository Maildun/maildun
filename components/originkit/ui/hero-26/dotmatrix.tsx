...
export default function DottedBackground({
    ...
    useEffect(() => {
        ...
        if (useGlyphAtlasFlag) {
            ...
            if (document.fonts && document.fonts.load) {
                document.fonts.load(fontCSS).catch(() => undefined);
            }
        ...
        return () => {
            ...
            if (resizeObserver) {
                try {
                    ...
                    // WebGL cleanup is best-effort during unmount.
                }
            ...
            if (glyphTextureRef.current) {
                try {
                    ...
                    if (typeof tex.destroy === 'function') {
                        tex.destroy();
                    } else if (typeof tex.delete === 'function') {
                        tex.delete();
                    }
                } catch {
                    // WebGL cleanup is best-effort during unmount.
        ...
        // Initialization intentionally runs once; later effects sync uniforms.
    ...
    useEffect(() => {
        ...
        if (perlin) {
            perlin.uniforms.uFrequency.value =
                mapFrequencyUiToShader(frequency);
            ...
            if (useGlyphAtlasFlag && glRef.current) {
                if (glyphTextureRef.current) {
                    try {
                        ...
                        // WebGL cleanup is best-effort while rebuilding glyphs.
                    }
