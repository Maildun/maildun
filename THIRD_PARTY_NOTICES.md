...
The application scaffolding — authentication pages and layouts, the appearance and sidebar hooks, and the settings pages — began as the [Laravel React starter kit](https://github.com/laravel/react-starter-kit), MIT License, Copyright (c) Taylor Otwell; it has since been modified substantially, and the original notice applies to the portions that remain.
...
## OriginKit dot-matrix component

`components/originkit/ui/hero-26/dotmatrix.tsx` is the "hero-26" dot-matrix component obtained from [OriginKit](https://www.originkit.dev), used as one of the subscribe-form artwork backgrounds. **It is not covered by Maildun's AGPL-3.0 license.** OriginKit retains all rights to the component and its design; the file is included under OriginKit's [Licensing & Usage](https://www.originkit.dev/docs/licensing) terms, which permit incorporating components into open-source applications with no attribution required, but do not permit redistributing a component on its own or as part of a template, theme, starter kit, UI kit, or component library.

If you fork Maildun, you may keep using and modifying the component as part of the application. You may not extract it for distribution as a component. Replace it with your own implementation if you need a codebase that is AGPL-3.0 throughout.

The component's fragment shader embeds the 3D simplex noise functions from [webgl-noise](https://github.com/ashima/webgl-noise), Copyright (C) 2011 Ashima Arts, MIT License; the full permission notice is kept in the file header. The component renders through `ogl` (Unlicense).
